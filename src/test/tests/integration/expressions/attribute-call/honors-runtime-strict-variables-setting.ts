...
const testCases: Array<[title: string, Record<string, any>, errorMessage: string]> = [
    ['record', {foo: {}}, 'TwingRuntimeError: Neither the property "bar" nor one of the methods bar()" or "getbar()"/"isbar()"/"hasbar()" exist and have public access in class "Object" in "index.twig" at line 1, column 4.'],
    ...
    ['null', {foo: null}, 'TwingRuntimeError: Impossible to access an attribute ("bar") on a null variable in "index.twig" at line 1, column 4.'],
    ['number', {foo: 5}, 'TwingRuntimeError: Impossible to access an attribute ("bar") on a number variable ("5") in "index.twig" at line 1, column 4.'],
];

for (const [name, context, errorMessage] of testCases) {
    for (const strictVariables of [false, true]) {
        runTest({
            description: `attribute call honors the runtime strict check setting (${strictVariables}) with ${name}`,
            templates: {
                "index.twig": `{{ foo.bar }}`
            },
