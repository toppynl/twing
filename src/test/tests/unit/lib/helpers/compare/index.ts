import {compare} from "../../../../../../main/lib/helpers/compare";
...
tape('compare', ({test}) => {
    test('should conform to PHP loose comparisons rules', ({same, end}) => {
        let rules = new Map([
            // TRUE
            ...
            // now
            [[dateNow, booleanTrue], true],
            [[dateNow, booleanFalse], false],
            [[dateNow, numberOne], true],
        ...
        for (let [operands, result] of rules) {
            same(compare(operands[0][1], operands[1][1]), result, `(${operands[0][0]} == ${operands[1][0]}) === ${result}`);
        }
