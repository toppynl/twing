...
tape('iterator-to-map', (test) => {
    ...
    test.test('should support custom value iterables', ({test}) => {
        test('iterable from a generator', ({same, end}) => {
            ...
            same([...iterable.keys()], [...expected.keys()]);
            same([...iterable.values()], [...expected.values()]);

            end();
        });

        test('explicit iterable', ({same, end}) => {
            let entry = createIterator();
