...
import {createParser, TwingParserOptions} from "../../../../../src/lib/parser";
...
import {TwingErrorSyntax} from "../../../../../src/lib/error/parsing";
...
import {TwingTagHandler} from "../../../../../src";
...
class TestTokenParser extends TokenParser {
    parse(_token: Token) {
        ...
        const stream = new TwingTokenStream([
        ...
        this.parser.parse(createStackEntry(stream));
...
// @ts-ignore
const getFilterBodyNodesData = (): Array<{ input: BaseNode, expected: BaseNode }> => {
    let input;
...
tape('parser', (test) => {
    ...
    test.test('subparse', (test) => {
        ...
        try {
            parser.parse(createStackEntry(new TwingTokenStream([
                ...
                new Token(TokenType.NAME, 'foo', 1, 0),
                new Token(TokenType.TAG_END, '{', 1, 0)
            ], new TwingSource('', 'foo'))), null, () => false);
    ...
    test.test('supports comment tokens', ({assert, end}) => {
        ...
        let node = parser.parse(createStackEntry(new TwingTokenStream([
    ...
    test.test('canOnlyAssignToNames', ({test}) => {
        ...
        for (const [templateSource, expectation] of templatesAndMessages) {
            test(expectation, ({same, fail, end}) => {
                ...
                try {
                    parser.parse(createStackEntry(env.tokenize(source)));
    ...
    test.test('arrayExpression', ({test}) => {
        ...
        for (let [source, expectation] of templatesAndNodes) {
            test(source, ({end, assert}) => {
                ...
                const moduleNode = parser.parse(createStackEntry(stream));
    ...
    test.test('macroDefinitionSupportsConstantDefaultValues', (test) => {
        ...
        for (const template of templates) {
            ...
            test.ok(parser.parse(createStackEntry(stream)), 'should not throw an error');
        }
    ...
    test.test('parseExpression', (test) => {
        ...
        let expression = parser.parseExpression(createStackEntry(stream));
    ...
    test.test('parseArguments', (test) => {
        test.test('with non-name named argument', (test) => {
            ...
            try {
                parser.parseArguments(createStackEntry(stream), true);
    ...
    test.test('parseFilterExpressionRaw', (test) => {
        test.test('deprecated filter', (test) => {
            ...
            for (let testCase of testCases) {
                ...
                parser.parseFilterExpressionRaw(createStackEntry(stream), createConstantNode(1, 1, 1), testCase[0]);
            }
