...
import {createModuleNode, ModuleNode} from "../../../../../src/lib/node/module";
...
class TwingTestsEnvironmentTestNodeVisitor extends TwingBaseNodeVisitor {
    ...
    protected doEnterNode(node: Node): Node {
        return node;
    }

    protected doLeaveNode(node: Node): Node {
        return node;
    }
}
...
class TwingTestsEnvironmentParserBar extends TwingParser {
    parse(): ModuleNode {
        return createModuleNode(
            createBodyNode(createTextNode('bar', 1, 1), 1, 1),
...
tape('environment', (test) => {
    test.test('autoescapeOption', async (test) => {
        ...
        let twing = createFilesystemAwareEnvironment(loader, {
            debug: true,
            cache: false,
    ...
    test.test('globals', async (test) => {
        ...
        twing = createFilesystemAwareEnvironment(loader);
        twing.setGlobal('foo', 'foo');
        await twing.loadTemplate('index');
        twing.setGlobal('foo', 'bar');
        test.same(twing.getGlobal('foo'), 'bar');
        ...
        twing = createFilesystemAwareEnvironment(arrayLoader);
        twing.setGlobal('foo', 'foo');
        twing.getGlobals();
        twing.getFunctions();
        await twing.loadTemplate('index');
        ...
        try {
            twing.setGlobal('bar', 'bar');
            test.fail();
        } catch (e) {
            test.false(twing.getGlobal('bar'));
        }
    ...
    test.test('testExtensionsAreNotInitializedWhenRenderingACompiledTemplate', async (test) => {
        ...
        let twing = createMockedEnvironment(loader, options);
    ...
    test.test('sourceMapChangeCacheMiss', async (test) => {
        ...
        sinon.stub(cache, 'generateKey').callsFake((_name, className) => {
    ...
    test.test('addExtension', (test) => {
        let environment = createFilesystemAwareEnvironment(new MockLoader());
        let ext = new TwingTestsEnvironmentTestExtension();

        environment.addExtension(ext, 'TwingTestsEnvironmentTestExtension');

        test.true(environment.hasExtension('TwingTestsEnvironmentTestExtension'));
        test.true(environment.getTags().has('test'));
        test.true(environment.getFilters().has('foo_filter'));
        test.true(environment.getFunctions().has('foo_function'));
        test.true(environment.getTests().has('foo_test'));
        test.true(environment.getOperator('foo_unary'));
        test.true(environment.getOperator('foo_binary'));

        let visitors = environment.getNodeVisitors();
        let found = false;
        ...
        test.test('with explicit name', (test) => {
            let twing = createFilesystemAwareEnvironment(new MockLoader());
    ...
    test.test('overrideExtension', (test) => {
        let twing = createFilesystemAwareEnvironment(new TwingLoaderArray({}));
    ...
    test.test('debug', async (test) => {
        class CustomEnvironment extends FilesystemEnvironment {
    ...
    test.test('autoreload', (test) => {
        let env = createFilesystemAwareEnvironment(new MockLoader(), {
            auto_reload: false
        });
    ...
    test.test('display', async (test) => {
        let env = createFilesystemAwareEnvironment(new TwingLoaderArray({
            index: 'bar'
        }));
    ...
    test.test('load', async (test) => {
        ...
        test.true(await env.loadTemplate(template));
        test.true(await env.loadTemplate('index'));

        test.end();
    });

    test.test('loadTemplate', async (test) => {
        let env = createMockedEnvironment(new TwingLoaderArray({index: 'foo'}), {
            cache: new MockCache()
        });
    ...
    test.test('source map support', (test) => {
        const cssIndexSourceName = 'css/index.css.twig';
        const cssColorSourceName = 'css/partial/color.css.twig';
        const cssBackgroundSourceName = 'css/partial/background.css.twig';
        const spacelessIndexSourceName = 'spaceless/index.html.twig';
        const spacelessPartialSourceName = 'spaceless/partials/foo.html.twig';

        const indexSource = `.foo {
    text-align: {{ align }};
    color: {% include "css/partial/color.css.twig" %};
    {% include "css/partial/background.css.twig" %}
    display: block;
}
`;

        const loader = new TwingLoaderArray({
            [cssIndexSourceName]: indexSource,
            [cssColorSourceName]: `whitesmoke`,
            [cssBackgroundSourceName]: `background-color: brown;
background-image: url("foo.png");
`,
            [spacelessIndexSourceName]: `foo
{% spaceless %}
    <foo>
...
{{ bar }}
    <foo>
        {% include "spaceless/partials/foo.html.twig" %}
    </foo>
{% endspaceless %}
oof`,
            [spacelessPartialSourceName]: `<FOO>FOO
{% spaceless %}
    {{ bar|upper }}
{% endspaceless %}
OOF</FOO>`
        ...
        test.test('when source_map is set to true', async (test) => {
            let env = createFilesystemAwareEnvironment(loader, {
                source_map: true
            });
            ...
            let sourceContent = consumer.sourceContentFor(cssIndexSourceName, true);

            test.same(sourceContent, indexSource);

            test.same(
                mappings,
                [
                    {
                        source: cssIndexSourceName,
                        generatedLine: 1,
                        generatedColumn: 0,
                        originalLine: 1,
                        originalColumn: 0,
                    ...
                    {
                        source: cssBackgroundSourceName,
                        generatedLine: 4,
                        generatedColumn: 4,
                        originalLine: 1,
                        originalColumn: 0,
        ...
        test.test('handle templates compiled without source map support', async (test) => {
            ...
            class CustomCache extends TwingCacheNull {
                generateKey(_name: string, className: string) {
                    return Promise.resolve(className);
                }
...
BAROOF</FOO></foo>oof`);
            ...
            test.same(
                ...
                [
                    {
                        source: spacelessIndexSourceName,
                        generatedLine: 1,
                        generatedColumn: 0,
                        originalLine: 1,
                        originalColumn: 0,
                    ...
                    {
                        source: spacelessPartialSourceName,
                        generatedLine: 4,
                        generatedColumn: 9,
                        originalLine: 1,
                        originalColumn: 0,
    ...
    test.test('getSourceMapNodeFactories', (test) => {
        ...
        test.true(factories.has("spaceless"));
    ...
    test.test('checkMethodAllowed', (test) => {
        ...
        env = createFilesystemAwareEnvironment(new TwingLoaderArray({}), {
            sandboxed: true
        });
