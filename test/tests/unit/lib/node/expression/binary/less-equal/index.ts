...
import {createLessThanOrEqualToNode} from "../../../../../../../../src/lib/node/expression/binary/less-equal";
...
tape('node/expression/binary/less-equal', ({test}) => {
    test('factory', ({same, end}) => {
        ...
        let node = createLessThanOrEqualToNode([left, right], 1, 1);

        same(node.children.left, left);
        same(node.children.right, right);
        same(node.type, 'less_equal');
