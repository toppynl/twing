...
import {createMatchesNode} from "../../../../../../../../src/lib/node/expression/binary/matches";
...
tape('node/expression/binary/matches', ({test}) => {
    test('factory', ({same, end}) => {
        ...
        let node = createMatchesNode([left, right], 1, 1);

        same(node.children.left, left);
        same(node.children.right, right);
        same(node.type, 'matches');
    ...
    test('compile', ({same, end}) => {
        ...
        same(compiler.compile(node).source, 'this.parseRegExp(2).test(1)');
