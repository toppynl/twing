...
This is a pnpm workspace. The root holds the original `twing` package (its sources under `src/`). Additional packages live under `packages/*`:
...
- `packages/components/` → published as `@toppynl/twing-components` (port of `symfony/ux-twig-component`: `{% props %}`, `{% component %}`, `<twig:…>` pre-lexer, `ComponentAttributes`). Peer-depends on `@toppynl/twing-html-extra` + `twing`.
...
- `pnpm run build:workspaces` — build `packages/*` (not root twing)
...
Versioning for `packages/*` is managed by Changesets (`.changeset/config.json`). The root `twing` package is listed under `ignore` and keeps its existing release flow.
